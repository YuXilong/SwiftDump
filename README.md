...
当前正式版本：[SwiftDump v1.2.2](https://github.com/YuXilong/SwiftDump/releases/tag/v1.2.2)
...
Formula 同时提供 `swift-dump` 命令别名。后续升级使用：

```sh
brew upgrade yuxilong/tap/swift-dump
```

正式 Release 发布后，Homebrew Formula 会由 GitHub Actions 自动发现、核对 `SHA256SUMS`、审计并更新；通常在一小时内同步，无需人工修改版本或校验值。
