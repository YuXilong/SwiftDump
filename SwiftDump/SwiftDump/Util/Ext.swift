...
extension Data {
    func hasReadableRange(offset: Int, length: Int) -> Bool {
        ...
        return length <= (self.count - offset)
    }

    func readableRange(offset: Int, length: Int) -> Range<Int>? {
        guard hasReadableRange(offset: offset, length: length) else {
            return nil
        }
    ...
    func readValue<Type>(_ offset: Int) -> Type? {
        guard let range = readableRange(offset: offset, length: MemoryLayout<Type>.size) else {
            return nil
        }
        let val:Type? = self[range].withUnsafeBytes { (ptr:UnsafeRawBufferPointer) -> Type? in
            return ptr.baseAddress?.loadUnaligned(as: Type.self);
        }
        return val;
    }

    func readCString(from: Int, maxLength: Int = 0x10000) -> String? {
        guard from >= 0, from < self.count else {
            return nil;
        }
        var address: Int = from;
        var result:[UInt8] = [];
        while address < self.count && result.count < maxLength {
        ...
        guard address < self.count, result.count < maxLength else {
            return nil;
        }
